	...
	methods: {
		...
		addCard(index) {
			...
			Object.keys(this.myCards).forEach((key, index) => {
				if (this.myCards[key].status == 0 && this.myCards[key].card.level <= this.equips[this.curequipIndex].equip.level) {
		...
		async chooseEquip() {
			...
			try {
				let tx = await bindEquip(this.runContract, this.equip.id)
				await tx.wait()
				uni.showToast({
					title: "绑定成功",
					icon: "success"
