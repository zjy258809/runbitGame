			...
			<uni-popup ref="inputDialog" type="dialog">
				...
				<uni-popup-dialog ref="inputClose" title="赛道选择" value="对话框预置提示内容!" placeholder="请输入内容"
					...
					<view>
						...
						<image :class="setTrackId == 0 ? 'ondialogimg' : 'dialogimg'"
							src="../../../static/Group12073.png" @click="chooseTrack(0)"></image>
						<image :class="setTrackId == 1 ? 'ondialogimg' : 'dialogimg'"
							src="../../../static/Group12072.png" @click="chooseTrack(1)"></image>
						<image :class="setTrackId == 2 ? 'ondialogimg' : 'dialogimg'"
							src="../../../static/Group11595.png" @click="chooseTrack(2)"></image>
					</view>
				</uni-popup-dialog>
				...
				provider.send("eth_requestAccounts", []).then(accounts => {
					...
					useContract(RunbitAddress, RunbitAbi).then(async runContract => {
						...
						getUnharvestReward(this.runContract, this.myAccount, this.curDay).then(
							estReward => {
								var value =ethers.utils.formatEther(estReward);
								console.log(value);
								this.reward =(parseFloat(value)).toFixed(2);
							})
						//获取装备,0代表没装备
						getBindEquips(this.runContract, this.myAccount).then(async equips => {
							// console.log("目前绑定的装备-----", equips)
							if (equips[this.setTrackId] == 0) {
								this.tip = this.setTrackId == 0 ? "石子路必须穿鞋子!" : this
									.setTrackId == 1 ? "芳草地必须穿裤子!" : "沙滩路必须穿上衣!"
								// this.messageText = this.tip
		...
		methods: {
			...
			getStep() {
				...
				myRequest({
					...
					this.getSteps = res.steps
					uni.setStorageSync('Steps', res.steps);
					getApp().globalData.userStep = this.getSteps;
					this.check_sum = res.check_sum
				})
			},
			//确认更新赛道
			setTrack() {
