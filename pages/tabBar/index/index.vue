	...
	export default {
						...
						useContract(cardAddress, cardAbi).then(contract => {
							...
							uni.getStorage({
								...
								if (!res[0]) {
									...
									//没缓存
									getMyCards(this.myAccount, contract).then(
										myCards => {
						...
						useContract(equipAddress, equipAbi).then(contract => {
							...
							uni.getStorage({
								...
								if (!res[0]) {
									...
									//没缓存
									getMyEquips(this.myAccount, this.equipContract)
										.then(myEquips => {
		...
		methods: {
			...
			async choosecard() {
				...
				try {
					let tx = await bindCard(this.runContract, this.currentequips.id, this.cardId, this.carIndex,this.gasPriceString);
					tx.wait().then(res => {
			...
			getEquitCardValue() {
				...
				this.aesthetic = 0
				for (let i = 0; i < 3; i++) {
					console.log("test", this.equips[i])
			...
			async dialogInputConfirm2(val) {
				...
				try {
					...
					let tx = await contract.addReferrerWithCheck(val, {
						gasLimit: 1200000,
